...
Building* vector_next(Vector* v, Building* current) {
    if (current == NULL || current - v->data >= v->size - 1) return NULL;
    return current + 1;
}

Building* vector_prev(Vector* v, Building* current) {
    if (current == NULL || current - v->data <= 0) return NULL;
    return current - 1;
}
...
void vector_insert(Vector* v, int index, const Building* build) {
